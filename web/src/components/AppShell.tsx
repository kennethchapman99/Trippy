...
export const AppShell = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row">
      {/* Sidebar */}
      <aside className="md:w-48 lg:w-52 md:min-h-screen md:sticky md:top-0 bg-card/85 backdrop-blur border-b-2 md:border-b-0 md:border-r-2 border-foreground/10 flex md:flex-col items-center md:items-stretch p-3 md:p-4 gap-2 z-40">
        <Link to="/" className="flex items-center md:mb-5 mr-auto md:mr-0">
          <img src={logo} alt="Trippy" className="h-14 w-auto object-contain" />
        </Link>
        <nav className="flex md:flex-col gap-1.5 ml-auto md:ml-0">
          {nav.map((n) => {
            ...
            return (
              <NavLink
                key={n.to}
                to={n.to}
                className={`flex items-center gap-3 px-3.5 py-2.5 rounded-2xl font-bold transition-bounce border-2 ${
                  active
